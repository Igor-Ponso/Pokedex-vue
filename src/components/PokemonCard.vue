...
<script lang="ts" setup>
  ...
  import { getPokemonTypePath, usePokemonStyles } from '@/composables/usePokemonUtils'
  ...
  const styles = usePokemonStyles(props.pokemon.types)

  const handleToggleShiny = () => {
    pokemonStore.toggleShiny(props.pokemon)
  }
</script>
...
<template>
  <v-card
    ...
    :style="styles"
    @click="$emit('show-details' as 'showDetail', props.pokemon.name)"
  >
    <v-card-title class="d-flex justify-space-between align-center pokemon-font">
      ...
      <v-btn icon @click.stop="handleToggleShiny">
        <v-icon>{{ props.pokemon.shiny ? 'mdi-star-outline' : 'mdi-star' }}</v-icon>
      </v-btn>
    </v-card-title>
    <v-card-subtitle class="d-flex justify-center">
      <transition name="fade">
        <v-img
          :key="props.pokemon.imageUrl"
      ...
      <div class="d-flex justify-center">
        ...
        >
          <img
            :alt="type.type.name"
            class="type-img"
            :src="getPokemonTypePath(type.type.name)"
          >
        </div>
      </div>
    </v-card-text>
